  ...
  return (
    <div className="p-6 bg-[var(--color-latar)] min-h-screen font-poppins">
      <div className="max-w-3xl mx-auto bg-white shadow-md rounded-xl p-6">
        ...
        {data.proof_url && <img src={data.proof_url} alt="Bukti" className="w-full max-h-60 object-contain rounded" />}
        <div className="mt-3">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={() => navigate(`/courses/form?id=${data.id}`)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded">Edit</button>
          <button onClick={handleDelete} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">Hapus</button>
        </div>
      </div>
    </div>
  );
